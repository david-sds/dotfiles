...
// Whether the active layout types dead accents, told from the layout/variant
// pair Hyprland lists rather than the description: the default national layouts
// (br, de, fr, ...) carry dead keys, plain English (us/en) does not, and an
// explicit `nodeadkeys` variant strips them regardless. Reading the variant that
// the active layout index points at keeps the split stable even when a layout
// pairs with an empty variant slot.
function hasDeadKeys(layoutList, variantList, index) {
  var layouts = String(layoutList || "").split(",")
  var variants = String(variantList || "").split(",")
  var layout = String(layouts[index] || "").toLowerCase()
  var variant = String(variants[index] || "").toLowerCase()

  if (variant === "nodeadkeys") return false
  if (layout === "us" || layout === "en") return /intl|international|euro/.test(variant)
  return true
}
