...
BarWidget {
  ...
  // The layout/variant pair the active layout index points at, so the label can
  // tell the dead-key and plain variants of the same language apart.
  property string layoutCodes: ""
  property string layoutVariants: ""
  ...
  readonly property string layoutLabel: KeyboardLayoutModel.shortLabel(layoutFull, layoutBriefs)
    + (KeyboardLayoutModel.hasDeadKeys(root.layoutCodes, root.layoutVariants, root.activeLayoutIndex) ? "·" : "")
  ...
  Process {
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        root.layoutCodes = String(kb.layout || "")
        root.layoutVariants = String(kb.variant || "")
        root.activeLayoutIndex = typeof kb.active_layout_index === "number" ? kb.active_layout_index : 0
      }
    }
  }
